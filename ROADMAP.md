...
| #354, #355 | Make the parse faster, and honest about its progress | Two halves. **The progress half is #354, built 11 September 2026**: upstream's byte offset inside each pass reaches the readout through one hook in `vendor/`, every pass an equal share, so it moves continuously instead of stepping 33 / 67 / 100 — and a container reports its own expansion, where the stage line used to say "decompressing" for the whole of the first pass. **The speed half is #355**, and the profile taken for #354 moved where the room is. #66 came first and is done; after it, **5.74 s of an 8.73 s native parse is entity decoding repeated once per pass**, so the ceiling is the three passes rather than anything inside one, and they are three because of two early returns in a copy this repository already patches. Whether lifting them yields identical output is the probe #355 opens with. Threads stay unavailable. | **P0** | M |
