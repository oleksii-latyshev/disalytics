...
worth having: it is complete after the first pass, while the second is still running.
...
**Labels move, tokens never do.** `labelPlacer` tries a ring of candidate positions around the token
and takes the first that clears every box already placed this frame; if every one of them collides it
keeps the first. It is reset per frame, so placement is a function of the frame and not of history.
The ring was the four cardinal boxes when this shipped and is **twelve** since #289 — the four, then
the corners, then rows above and below — and a label placed outside the cardinal four is *displaced*,
which since #290 is what earns it a hairline back to its own token.
...
- **Objectives:** plant, defuse start/complete/abort, site, and the tick the planted bomb went off —
  `null` when it never did. `mp_c4timer` is not among the convars a recording carries, so the
  interval between a plant and its own detonation is the only measurement of the bomb's timer the
  demo offers (`docs/PARSER.md` §21)
- **Rounds:** start/end tick, winner, win reason, freeze-time end, and the round's own length in
  seconds — read from the engine's `m_iRoundTime` at freeze-time end, and `null` where the demo does
  not carry that prop. It is per round because a config change between halves or into overtime moves
  it, and it is what lets the clock count *down* the way the game does
...
`image/png`. **Twelve assertions**, and which side of the wire each one gets its path from is the
difference between a true and a false failure:

- **Hashed assets — the deployed page.** The script fetches `/`, then follows what the page names:
  `index.html` names the entry chunk and the stylesheet, the entry chunk names the parse worker, the
  worker names the binary, and the stylesheet names the fonts. Nothing in `index.html` mentions the
  `.wasm` at all, so nothing shorter than that walk finds it. One asset per extension is then
  requested — `_headers` grants `immutable` to `/assets/*` by path, so the contract is about the
  directory and a sample per kind asserts it.
- **The radar image — `packages/map-data/assets/radar`.** Those images are committed and copied
  verbatim rather than content-hashed, so their served paths are the same on every machine, which
  is what makes a local listing an honest source for them.
- **Nothing reads `apps/web/dist`,** and the script needs no local build at all.

It used to derive every hashed path from `apps/web/dist`, and the claim that this "always tests the
build that was shipped" was false off CI. **A local asset name and a deployed one can never
agree**: measured on `1605518`, a macOS `wasm-pack` build and the Linux one `deploy.yml` ships are
the same size from the same source and pinned to the same `wasm-pack` 0.15.0, and are not the same
bytes — so Vite content-hashes them differently. Asking production for the local name got the SPA
shell, and the `wasm content-type` assertion failed against a deploy whose own smoke run was green.
Inside `deploy.yml` the old script was right, because there the `dist` it read was the `dist` that
had just been deployed; it was wrong exactly when a human or an agent verified a deploy
independently of its own badge.

**One request per asset**, and both the caching and the content-type assertions read that single
response. They used to make their own requests seconds apart and could disagree about the same URL —
`immutable .wasm` passing while `wasm content-type` failed, which is #65's own evidence and is not a
state a deployment can be in.

**A hashed asset whose name is new in this deploy lags the document that names it**, measured at
38 s and again over 2.5 minutes on deploys that were correct throughout. The script waits for one
to stop being answered by the shell, on a deadline **shared across all four assets** rather than
granted to each — propagation is one event at the edge. A shell answering an asset path is
retryable; an asset still not there at the deadline is a failure, and so is a shell counted as
`immutable`, which `_headers` grants by path and which is why that assertion used to pass on a file
that was not being served.

The thirteenth assertion this section used to count was `immutable .map`. A source map is not part
of the contract and nothing the app runs asks for one, so the walk does not follow one.
...
request, so the wait is what makes the difference legible. The asset wait above is the same lag one
level down, and it went unnoticed for as long as it did because a commit touching both halves
triggers `deploy` twice (§15) — which looks like a retry policy and is not one.
...
| Metric | Budget | Note |
|---|---|---|
| Parse a 300 MB demo | < 15 s | Set from Phase 0 and **measured properly by #59**: in a foreground Chrome tab, off disk, drop to summary, three runs per input on an idle machine. **The shipped profile is `-O3` since #66 and this budget is met**: **11.11 s** on the 353 MB raw fixture and **14.19 s** on the 264 MB `.dem.zst`, where `-Oz` took 15.29 s and 18.85 s and was over on both — the container being what most people have. The 0.40 MB it costs the WASM row below is what buys it, and it is the only configuration measured where both budgets hold at once. **#66 re-measured the container arm on the built bundle** a month later, four interleaved passes of three runs inside one hour, drop to review screen: `-Oz` **21.92 s** and **21.37 s**, `-O3` **14.91 s** and **16.19 s**. Both arms sit above #59's day — the baseline reproduced itself to 0.55 s across the hour, so the machine was slower rather than drifting — and the 28% between them is #59's 25% again, with the branch's best pass at 14.48 / 14.48 s reproducing #59's 14.19 s. **It is met with no headroom**: the same build read 17.23 s on the slowest run of that hour, which is why this row is a budget and not a promise. **#355 gave it headroom** by taking the parse from three passes to two: the same method on 11 September 2026, arms interleaved in one hour, read `main` at **15.72 s** — over, on a slower day — and the branch at **13.45 s**, its slowest run 14.17 s. Measure from the drop: worker spawn plus `init()` is 0.08 s cold. Two things this number does *not* cover — a tab the user switches away from, which is 5× slower because Chrome backgrounds the renderer, and genuinely slow hardware, which `docs/PARSER.md` §11 keeps open. Full method and breakdown in `docs/PARSER.md` §16; the earlier native, Bun and blob-fed figures are §9, §14 and §15. **Re-measured on schema 5's shot array** (#163): three browser runs each side of the change on one machine, **19.94 s before and 20.06 s after** — inside the spread of either arm, so 3,500 more events cost nothing measurable, and the native three-pass parse moved 16.31 s → 16.09 s the other way. Both browser arms sit ~1.1 s above the 18.85 s above, which is the day and not the change; the figure in this row stands as #59 measured it. **Re-measured on schema 7's rules prop** (#295), which puts one more column on the ticks pass: three native three-pass runs an arm on one machine within the hour, **16.81 / 16.54 / 16.55 s** before and **15.41 / 15.31 / 16.31 s** after. The arms overlap and the branch reading lower is the spread, not a gain from adding a column; the figure in this row is unmoved. |
| Peak tab memory during parse | < 1.5 GB | 663 MB of WASM linear memory measured, ~1.0 GB estimated whole-tab. True tab memory cannot be measured without cross-origin isolation, which §13 forbids. A container adds a second copy while it is expanded — 264 MB compressed beside 353 MB expanded, **617 MB**, under what the parse itself reaches. It stays a transient because the compressed file is freed before the passes rather than after them (`docs/PARSER.md` §15). Linear memory ends at its peak, so it can be read after a parse in Bun: **849 MiB** on the container fixture, and **897 MiB** since #355, whose merged pass holds every event's copy of the tick props beside the tick columns (`docs/PARSER.md` §25). |
| Timeline scrubbing | 60 fps sustained | **Held with room to spare** — 399 frames dragging the scrubber across the whole match on the §5 layout: **0 over 16.7 ms**, median 8.3 ms, p95 9.2 ms, worst 9.3 ms. Headed Chrome over CDP against the built bundle at 1440×900 on the 264 MB fixture, `document.visibilityState` asserted inside the run; the median is the panel's own 120 Hz rather than a ceiling this hits. Measured by the layout issue. Re-measured on §6.1's plate states (#153): median 8.3 ms and p95 9.3 ms unchanged, with **0–1 frames over 16.7 ms per pass on the branch and the same on `main`** — a single worst-frame figure is noise at this resolution, and the count across a few hundred frames is the number that survives. **Re-measured on the scoreboard brow** (#196, the first re-measurement since #147, so it also carries #182's and #184's timeline work): three passes of 399 frames, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.2–9.3 ms, worst 9.4 ms, the panel at 120 Hz in all three. What is dragged is no longer the same control: since #182 the scrubber is §7.1's *round* axis, so a pass is one round end to end rather than the whole match. **Re-measured on §5.4's event feed** (#159): three passes of 399 frames driving that axis end to end with the feed live, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3 ms, worst 9.3–9.4 ms, the panel at 120 Hz in all three. **Re-baselined on `320f821`** (24 August 2026, no PR of its own): three passes of 399 frames driving the axis end to end, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.2–9.3 ms, worst 16.6 ms, 120 Hz. That is the first measurement covering #199 through #221 — the settings store, the plate legend, the match overlay and the round strip — and it moved nothing. **Re-measured on §6.1's three new token marks** (#164 — a weapon silhouette beside every name, a hollow for a walking player, a spur for a shot): three passes of 399 frames, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.2–11.4 ms, worst 9.7–14.2 ms, 120 Hz. A `main` (`3b29b62`) baseline taken the same hour with the identical script read 0 over, median 8.3 ms, p95 9.2–9.3 ms, worst 9.4–9.5 ms — so the one pass with an 11.4 ms p95 is the noise this row has always said a single figure is, and the marks cost nothing measurable. **Re-measured on §8's assembly** (#104): three passes of 399 frames driving the axis end to end, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3 ms, worst 9.4 ms, 120 Hz, with the scrubber's own `value` sampled every frame so each pass records the span it dragged (1242 → 2339, one round end to end, in all three) rather than only the clock it started and finished on. A `main` (`d7c311b`) baseline the same hour with the identical script read the same figures across three passes, p95 9.1–9.3 ms. **Re-measured on the three grenade functions** (#172): three passes of 399 frames driving the axis end to end, **0, 0 and 1 frame over 16.7 ms**, median 8.3 ms, p95 9.2–9.3 ms, worst 9.4/9.4/16.8 ms, 120 Hz, with the span recorded per pass (1822 → 2343, 2343 → 3215, 3215 → 5086). A `main` (`7e3967f`) baseline the same hour with the identical script threw the same single 16.8 ms frame in a *different* pass — 1, 0 and 0 over — across those same three spans, which is this row's own standing point about what a single worst-frame figure is worth. **Re-measured on the redressed stage** (#278): three passes of 399 frames driving the round axis end to end, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.0–9.1 ms, worst 9.3–9.4 ms, 120 Hz, each pass seeking to a fresh round first so it drags a control that can actually move — the span is recorded per pass (8006 → 10104, 10434 → 11274, 11599 → 13105) and a pass that started parked at a round's end would otherwise have measured nothing. A `main` (`bbb25f5`) baseline the same hour with the identical script and the same three spans: 0 over, median 8.3 ms, p95 9.2–9.3 ms, **worst 16.6 ms in two of the three passes** — so the arm carrying ten rolling money figures is the one with the *quieter* worst frame, which is this row's own standing point about what a single worst-frame figure is worth. **Re-measured on the plate's own vocabulary** (#286): three passes of 399 frames driving the round axis end to end, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.0–9.1 ms, worst 9.3–9.4 ms, 120 Hz, each pass seeking a fresh round first and recording its own span (11279 → 13112, 16577 → 17626, 20208 → 21122). A `main` (`8d69561`) baseline the same hour with the identical script and those same three spans: 0 over, median 8.3 ms, p95 9.0–9.1 ms, worst 9.4–9.5 ms. The two arms are indistinguishable. **Re-measured on the plate's label size** (#282): three passes of 399 frames driving the round axis end to end, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3 ms, worst 9.4 ms, the same three spans (11279 → 13112, 16577 → 17626, 20208 → 21122). A `main` (`aad966e`) baseline the same hour with the identical script: the same figures on every row. **Re-measured on the redressed timeline block** (#279 — the round strip as a toggle group with a sliding highlight, the survivor tracks inside the pill, and a per-glyph collapse on the axis being dragged): three passes of 399 frames driving the round axis end to end, **0, 0 and 1 frame over 16.7 ms**, median 8.3 ms, p95 9.3 ms, worst 9.4/9.4/16.7 ms, 120 Hz. A `main` (`1e749fd`) baseline the same hour with the identical script: **1, 0 and 0 over**, median 8.3 ms, p95 9.3 ms, worst 16.7/9.4/9.4 ms — the same single frame in a different pass, which is this row's own standing point about what a worst-frame figure is worth. What is dragged draws fewer of its marks as symbols than it did and the same number of elements either way: a collapsed glyph is a `span` where a symbol was an `svg`, and which one it is is decided once per round in the `useMemo` that already sized the hit slots. **Re-measured at the close of the redesign** (#280 — the settings sheet as an accordion of five toggle groups, the help legend on the plate's own ground, and the two transitions that named a property their element never set): three passes of 399 frames driving the round axis end to end, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3 ms, worst 9.9/13.5/10.6 ms, the span recorded per pass (6365 → 7686, 15145 → 16577, 21122 → 22561). A `main` (`9d95208`) baseline the same hour with the identical script and the same three spans: 0 over, median 8.3 ms, p95 9.4–9.6 ms, worst 12.7–13.9 ms. The two arms are indistinguishable, and nothing in that PR is on this path at all — the sheets exist only while playback is stopped, and the two transitions are on a knob and a thumbnail. The plate measured **716 in both arms**. **Re-measured on the round clock's direction** (#295 — a rules prop on the ticks pass, `SCHEMA_VERSION` 7, and a scoreboard that switches phase and colour on the 10 Hz readout): three passes of 399 frames driving the round axis end to end, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.4–9.5 ms, worst 11.6–12.3 ms, the span recorded per pass (7807 → 9993, 24338 → 26592, 45279 → 47078). A `main` (`2affee5`) baseline the same hour with the identical script and the same three spans: 0 over, median 8.3 ms, p95 9.4–9.7 ms, worst 11.2–13.4 ms. The two arms are indistinguishable. **A third run is on the record and discarded, and why matters more than the numbers**: the branch's *first* arm read 0/0/1 over with p95 12.2–12.8 ms, and it ran on a page that had already been driven through two locales, a full-tree redaction pass and thirty seeks. `main` and the branch's repeat were both taken on a page freshly dropped-into and agree exactly; a measurement here is only comparable to one whose page has had the same history. **Re-measured on the grenade feed row** (#310 — a fourth row kind, and a hovered row that draws that grenade's own path on the plate): three passes of 399 frames driving the round axis end to end, **0 over 16.7 ms in every one**, median 8.3 ms, worst 9.4 ms, 120 Hz. A `main` (`9992173`) baseline the same hour with the identical script: the same figures on every row. What the branch adds to this path is one ref read per frame and one integer comparison per visible grenade, and the hovered path itself is a `trajectoryClipCount` the layer already calls for a grenade in flight. **Re-measured on the round axis's filter** (#313): three passes of 399 frames driving the round axis end to end, **1, 0 and 0 frames over 16.7 ms**, median 8.3 ms, p95 10.0–10.3 ms, worst 19.7/13.9/13.9 ms, 120 Hz, the span recorded per pass (10434 → 10114, 21442 → 22561, 35002 → 36954). A `main` (`3e20603`) baseline the same hour with the identical script and the same three starting rounds: 0 over in every pass, median 8.3 ms, p95 10.1–10.3 ms, worst 12.9–14.2 ms. The single frame is this row's own standing point about a worst-frame figure, and what is dragged is *fewer* elements than before whenever a facet is off — the filter is a `useMemo` on the same list `axisGlyphs` already derives once per round, and it runs before the hit slots are sized rather than beside them. **The axis is the same width on both arms** — 886.55px at 1440×900 and 518.55px at 1024×800 — because the control sits in the round strip's row and not in the axis's. **Re-measured on the zoomed plate** (#315 — the canvas is the whole stage while the reader is zoomed in, 1392×657 against 716×716 at 1440×900, which is 1.78× the pixels): three passes of 399 frames driving the round axis end to end **with the plate expanded**, **1, 0 and 0 frames over 16.7 ms**, median 8.3 ms, worst 17.0/15.0/10.4 ms, 120 Hz. The same three passes at rest on the same build: **1, 1 and 0 over**, median 8.3 ms, worst 16.7/16.7/10.4 ms. A `main` (`2615053`) baseline the same hour with the identical script: **1 frame over in every pass**, median 8.3 ms, worst 16.7/17.8/16.7 ms — so the arm with nearly twice the canvas is the *quieter* one, which is this row's own standing point about what a single worst frame is worth. The plate measured **716 at rest and 1392×657 expanded at 1440×900**, and **473 against 1024×473 at 1024×800**, in `en` and in `ru`. **Re-measured on the hit's own figure** (#287 — a running total beside every living token, placed through the same placer the names are): three passes of 399 frames driving the round axis end to end, **0, 0 and 1 frame over 16.7 ms**, median 8.3 ms, p95 9.3 ms, worst 9.4/15.7/16.8 ms, 120 Hz. A `main` (`8eaa234`) baseline the same hour with the identical script: **1, 0 and 0 over**, median 8.3 ms, p95 9.1–9.3 ms, worst 16.7/9.4/9.4 ms — the same single frame in a different pass, which is this row's own standing point about what a worst-frame figure is worth. The plate measured **716 at 1440×900 and 473 at 1024×800** with **0 elements overflowing**: the mark is inside the canvas, so §5.1 cannot see it. **Re-measured on #290's leader line** against a `main` (`d90cb0f`) baseline the same hour, three passes of 399 frames an arm driving round 7's axis end to end from its buy phase — ten labels on the plate, three of them displaced and so carrying a line: **0 over 16.7 ms in every pass on both arms**, median 8.3 ms, p95 9.3 ms, worst 9.4 ms. The plate is **716 at 1440×900 and 473 at 1024×800** with **0 elements overflowing**, unchanged. |
| Review screen, playing, everything on | 60 fps sustained | The assertion §17 rules 1 and 3 are written against, and the thing that decides whether the material in that document survives contact with the loop. **Held** — 399 frames of playback on the §5 layout with all four cards under `backdrop-filter` and ten tokens with names on the plate: **1 over 16.7 ms**, median 8.3 ms, p95 9.2 ms, worst 17.0 ms. Same method as the scrub row above. **Re-measured on §6.1's plate states** (#153 — 16px tokens for both sides, a blind countdown disc, a death that shrinks, a selection ring, and names stroked with a halo instead of filled behind a chip): 0–1 over 16.7 ms across two passes, median 8.3 ms, p95 9.3 ms — and `main` measured with the identical script the same afternoon threw the same occasional single frame, so the states cost nothing measurable. **Re-measured on the scoreboard brow** (#196): three passes of 399 frames of playback against the built bundle, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.1–9.3 ms, worst 9.3–9.4 ms, 120 fps, with the clock read from the scrubber's `aria-valuetext` (01:43 → 01:58 across the three) to prove playback was actually running. That pass covers §6.2's utility and #182's and #184's timeline, none of which had been measured. It also changes what the row asserts: with the brow as the default position there is **no `backdrop-filter` over the plate at all**, so the exception §2.3 grants the scoreboard chip is now spent only by readers who ask for it — and #187 had already found that exception indistinguishable from no blur across nine arms. **Re-measured on §5.4's event feed** (#159), which was the last thing this row did not cover: three passes of 399 frames of playback against the built bundle, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.2–9.3 ms, worst 9.4 ms, 120 fps, with the clock read from the scrubber's `aria-valuetext` (02:18 → 02:29 across the three) to prove playback was running. A fourth pass sampled the feed's own row count every twentieth frame *during* the arm rather than after it — 2 rows rising to 4 as they arrived — because a pass that happens to end on a round change reads an empty feed and says nothing about what was on screen while it was measured; that is the arrival animation running inside the budget rather than beside it. When it does fail, the blur is the first thing to go, then the feed's motion; the layout is not what gets cut, because the layout is what makes the blur affordable in the first place (§5.1). **Re-measured on §6.3's zoom** (#114): three passes of 399 frames of playback **at the 4× ceiling**, where the backdrop is being enlarged and the tokens are at their 20px maximum, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3 ms, worst 9.4 ms, 120 fps, the clock read off the scrubber (11:04 → 11:14 across the three) to prove playback was running. A `320f821` baseline taken the same day with the identical script read the same figures at 1×, so the zoom costs nothing measurable: it multiplies one scalar per layer per frame and translates the context, and the number of marks drawn does not change with it. **Re-measured on §6.1's three new token marks** (#164), which is the change this row exists for — three marks per token is exactly what spends a plate's budget: three passes of 399 frames of playback, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.1–9.3 ms, worst 9.4–9.6 ms, 120 fps, the clock read off the scrubber (03:19 → 03:30 across the three) to prove playback was running. A `main` (`3b29b62`) baseline the same hour: 0 over, median 8.3 ms, p95 9.3 ms, worst 9.5–9.8 ms, 02:25 → 02:36. The two arms are indistinguishable. What keeps them so is that all three marks are conditionals inside a loop that already ran: the weapon silhouettes are compiled to `Path2D` once per class per session, the gunfire lookup is `damageFlashBySlot`'s own shape into the layer's own array, and the hollow is one more `arc`. **Re-measured on §9.3's hot corners** (#246), which put a `pointermove` listener on a path that fires while the match plays: three passes of 399 frames of playback **with the pointer sweeping the stage all the way through each pass**, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.2–9.3 ms, worst 9.4 ms, 120 fps, the clock read off the scrubber (01:43 → 01:54 across the three) to prove playback was running. A `main` (`0aa9d03`) baseline the same hour with the identical script and the same sweep: 0 over, median 8.3 ms, p95 9.2–9.3 ms, worst 9.4 ms, the same 01:43 → 01:54. The two arms are indistinguishable, which is what a handler of four comparisons and three ref writes should look like — the listener sets React state on a *boundary crossing* and never on a move, and the stillness check is one 500 ms interval rather than a `setTimeout` reset per event. **Re-measured on §8's one orchestrated moment** (#104 — the plate fading up, the four cards arriving from the edges they live against, and §7.3's strip filling left to right): three passes of 399 frames of playback, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3 ms, worst 9.4 ms, 120 fps, the clock read off the scrubber (01:44 → 01:55 across the three) and its frame span recorded per pass, so a pass that measured a paused screen could not pass for one that measured playback. A `main` (`d7c311b`) baseline the same hour with the identical script: the same figures and the same 01:44 → 01:55. The sequence itself cannot reach this row — it is a mount transition, over 600 ms before anything plays, on a clock that is paused throughout — and what outlives it is 30 `m.li` in the round strip that reconcile once a round rather than once a frame, and inline `opacity: 1` / `transform: none` with no `will-change` left behind on any of the 57 elements the run swept. The plate measured **716 in both arms** at 1440×900, so the arrivals cost §5.1 nothing either. **Re-measured on the three grenade functions** (#172), which is the change this row exists to catch — the utility layer's draw loop hands each grenade to a module-level function now, taking one `UtilityDraw` allocated per demo instead of closing over eleven values: three passes of 399 frames of playback, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.0–9.3 ms, worst 9.4 ms, 120 fps, the clock read off the scrubber (01:43 → 01:53 across the three) to prove playback was running. A `main` (`7e3967f`) baseline the same hour: the same figures and the same 01:43 → 01:53, plate **716 in both arms**. The two arms are indistinguishable, which is what a shape change with no behaviour in it should look like. **Re-measured on the redressed stage** (#278 — a health wash per row, ten `SlidingNumber` money figures, and the selected player's round drawn under their own name on the plate): three passes of 399 frames of playback with a player selected throughout, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.0–9.2 ms, worst 9.4–9.5 ms, 120 fps, the clock read off the scrubber (08:20 → 08:32 across the three) and the frame span recorded per pass to prove playback was running. A `main` (`bbb25f5`) baseline the same hour: 0 over, median 8.3 ms, p95 9.1–9.3 ms, worst 9.3–10.1 ms, the same 08:20 → 08:32. The two arms are indistinguishable. **What `SlidingNumber` costs is DOM rather than frames**, and it is worth stating: the screen goes from 1,143 elements to 1,654, of which 451 are the roller's own — 41 digit places, each ten absolutely-positioned copies with a `useTransform` on them, plus a `ResizeObserver` per place. The budget holds with all of it on, which is what the issue asked to be checked rather than assumed. The plate's own detail line costs nothing measurable: it is one more `strokeText`/`fillText` pair for one slot, and its width is measured on the frame the string changes and cached after it, so a draw never calls `measureText`. **Re-measured on the plate's own vocabulary** (#286 — Valve's model outline beside every name instead of a class silhouette, Valve's own object for a piece of held utility, and the grenade itself drawn at the head of every trajectory in flight): three passes of 399 frames of playback with a player selected throughout, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.0–9.1 ms, worst 9.3–9.4 ms, 120 fps, the clock read off the scrubber (01:43 → 01:55 across the three) and the frame span recorded per pass (1662 → 1722, 1722 → 1781, 1781 → 1841) to prove playback was running. A `main` (`8d69561`) baseline the same hour with the identical script: 0 over, median 8.3 ms, p95 9.0 ms, worst 9.4 ms, the same three spans, plate **716 in both arms**. The two arms are indistinguishable, and what keeps them so is that nothing new happens per frame — a mark is a `Path2D` compiled once per weapon per session and drawn where a mark was already being drawn, and the grenade in flight is one more `fill` on a projectile the layer was already walking for its trajectory. **Re-measured on the plate's label size** (#282 — a name 30% larger, a mark box 24×10 → 31×13, and a placer trying twelve candidate boxes where it tried four): three passes of 399 frames of playback with a player selected throughout, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.2–9.3 ms, worst 9.4 ms, 120 fps, the clock read off the scrubber (01:43 → 01:55 across the three). A `main` (`aad966e`) baseline the same hour: the same figures and the same clock, plate **716 in both arms**. Neither half of that change is a per-frame cost: bigger type is the same two calls per label with more glyph area inside them, and the eight extra candidate positions are reached only by a label whose first four are taken — at most eight more rectangle comparisons for one of ten labels, against the ten the placer already made. **Re-measured on the redressed timeline block** (#279): three passes of 399 frames of playback with the block rebuilt — 30 toggle buttons in a composite group, a `layoutId` highlight under the round being played, and the survivor tracks inside the pill — **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3 ms, worst 9.4/9.4/16.7 ms, 120 fps. A `main` (`1e749fd`) baseline the same hour with the identical script: 0 over in every pass, median 8.3 ms, p95 9.2–9.3 ms, worst 9.4–9.5 ms. The two arms are indistinguishable, and what keeps them so is that nothing on the strip is a function of the frame: the lit round turns over once a round on the 10 Hz readout, `RoundOutcomes` is still `memo`'d against it, and the highlight animates on `transform` when it moves. The plate measured **716 in both arms** — and 716 in *both* survivor states on the branch, against 700 with the tracks showing on `main`, which is the 16px the block used to take out of the map. **Re-measured at the close of the redesign** (#280): three passes of 399 frames of playback, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3–9.4 ms, worst 10.2–10.7 ms, the clock read off the scrubber (01:43 → 01:55 across the three) and the frame span recorded per pass (1662 → 1722, 1722 → 1781, 1781 → 1841) to prove playback was running. A `main` (`9d95208`) baseline the same hour with the identical script: 0 over, median 8.3 ms, p95 9.5–10.9 ms, worst 12.5–14.4 ms, the same clock and the same three spans, plate **716 in both arms**. The branch is the quieter arm on both tail figures, which is this row's own standing point about what a single worst-frame figure is worth rather than a claim that an accordion made the plate faster. That is the last measurement the redesign owes: every screen in the product has now been measured on the new system, and the milestone moved neither of these two rows. **Re-measured on the round clock's direction** (#295): three passes of 399 frames of playback with a player selected throughout, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.4–10.7 ms, worst 9.8–13.6 ms, the frame span recorded per pass (8177 → 8230, 24719 → 24772, 45584 → 45637) and the clock read off the scrubber to prove playback was running. A `main` (`2affee5`) baseline the same hour with the identical script: 0 over, median 8.3 ms, p95 9.3–11.0 ms, worst 11.1–14.4 ms, the same three spans, plate **716 in both arms**. Nothing in that change is on the frame path — the clock is `roundClockAtFrame` at the 10 Hz readout, and what it does per call that it did not before is one binary search over 22 plants. See the scrub row above for the discarded first arm and what it says about comparing two pages with different histories. **Re-measured on the grenade feed row** (#310): three passes of 399 frames of playback, **0 over 16.7 ms in every one**, median 8.3 ms, worst 9.4 ms, 120 fps. A `main` (`9992173`) baseline the same hour with the identical script: the same figures on every row. The plate measured **716 at 1440×900 and 473 at 1024×800**, in `en` and in `ru`, matching what #308 measured on the arm this branch came off — a row kind added to a floating card is not in the rows §5.1 sizes the plate from. **Re-measured on the round axis's filter** (#313): three passes of 399 frames of playback, **0 over 16.7 ms in every one**, median 8.3 ms, p95 10.3–10.4 ms, worst 12.3–13.7 ms, 120 fps, the clock read off the scrubber (10:52 → 10:55, 22:20 → 22:23, 36:27 → 36:31) and the frame span recorded per pass to prove playback was running. A `main` (`3e20603`) baseline the same hour with the identical script: 0 over, median 8.3 ms, p95 10.1–10.3 ms, worst 12.0–12.6 ms, the same clock and the same three spans. The two arms are indistinguishable, and nothing in the change is on this path at all: the filter is read once per render of a component that rides the 10 Hz readout, and the axis's glyph list turns over once a round. The plate measured **716 at 1440×900 and 473 at 1024×800**, in `en` and in `ru`, on both arms, and the timeline block **92px** (124 with the scoreboard brow) on both. **Re-measured on the zoomed plate** (#315): three passes of 399 frames of playback **with the plate expanded**, **0 over 16.7 ms in every one**, median 8.3 ms, worst 10.4 ms, 120 fps, the clock read off the scrubber (01:44 → 01:47, then 02:12 → 02:16 twice) to prove playback was running. The same three passes at rest on the same build: 0 over, median 8.3 ms, worst 9.4 ms, the same clock. A `main` (`2615053`) baseline the same hour with the identical script: the same figures again. Three arms indistinguishable, and what keeps them so is that a bigger plate draws no more marks than a smaller one — the extra pixels are fill on a `drawImage` the backdrop layer was already making, and every mark above it is measured in device pixels and so is the same size on both. The plate measured **716 at rest against 1392×657 expanded at 1440×900**, and **473 against 1024×473 at 1024×800**; the team cards, the timeline block and the corner did not move in any state, in either locale, with **0 elements overflowing** on the sweep over `document.querySelectorAll('*')`. **Re-measured on the hit's own figure** (#287): three passes of 399 frames of playback, **0 over 16.7 ms in every one**, median 8.3 ms, p95 9.3 ms, worst 9.4 ms, 120 fps. A `main` (`8eaa234`) baseline the same hour with the identical script: the same figures on every row. **A third arm played through the fixture's densest moment for this mark**, which is the arm that means anything here: the ordinary rows start in round 2 where nobody has been hit yet, so a mark that exists only during a fight would otherwise have been measured against a plate drawing none of it. Round 27, frame 42,473 — four figures standing at once, and no frame of the match holds five — entered 40 frames early and played through: **0 over 16.7 ms in three passes on each arm**, median 8.3 ms, worst 9.4 ms. What the branch adds per frame is one forward walk over the damage events of the last twelve seconds and at most four `strokeText`/`fillText` pairs, and the figure's width is its character count against two widths measured once per demo, so a draw never calls `measureText`. **Re-measured on #290's leader line** against the same `d90cb0f` baseline, three passes of 399 frames an arm playing out of round 7's buy phase: **0 over 16.7 ms in every pass on both arms**, median 8.3 ms, p95 9.1–9.3 ms, worst 9.4–9.5 ms, the clock walking 10:35 → 10:38 on every pass of both. What the branch adds per frame is one 1px stroke per *displaced* label — three of ten in that window, and never more than four measured anywhere on the fixture. |
...
| JS bundle, excl. WASM | < 500 kB gzip | single locale only. **233.97 kB, 46.8% of budget** — measured on a forced clean build (`rm -rf apps/web/dist && bun run build --force && bun run size`) at #313, against **233.09 kB** for `main` at `3e20603` on a clean build the same hour. The 0.88 kB is the round axis's filter: one component, one hook, one pure helper, six strings in each locale and a lucide icon that had not been imported before — **no new dependency**, because the toggle group and the composite under it have shipped since #279 and this is a third caller for them. The 233.09 kB figure and its history follow. 233.09 kB — measured the same way at #310, against **232.85 kB** for `main` at `9992173` on a clean build the same hour. The 0.24 kB is a fourth kind of feed row and the helper that bounds its life: no new art, no new dependency, and one string moved between two namespaces. **This line had read 232.95 kB since #280 and was a figure for no tree in between** — #295, #308 and #309 are the 0.10 kB it never recorded, and it is a *fall* rather than a rise, which is the shape of a stale figure that looks harmless; #174 is the issue that says one here is a defect rather than a lag. The 232.95 kB figure and its history follow. 232.95 kB — measured on a forced clean build (`rm -rf apps/web/dist && bun run build --force && bun run size`) at #280, against **229.21 kB, 45.8%** for `main` at `9d95208` on a clean build the same hour. The 3.74 kB is Base UI's accordion and nothing else: the settings sheet is the first caller for it, and the toggle group its choice rows moved onto had already arrived with #279's round strip — a second consumer of a component that has shipped costs this line nothing measurable. That figure closes the redesign, and it is the last row of it: the milestone's whole cost against the tree it started from is **195.08 → 232.95 kB**, one runtime dependency, five screens. The 229.21 kB figure and its history follow. 229.21 kB — measured on a forced clean build at #279, against **223.50 kB, 44.7%** for `main` at `1e749fd` on a clean build the same hour. The 5.71 kB is Base UI's toggle group and the composite root and item under it — the roving focus, the pressed state and the arrow keys that the round strip and the speed control both stopped implementing for themselves — arriving with the registry's first caller for them. The 223.50 kB figure and its history follow. 223.50 kB — measured on a forced clean build (`rm -rf apps/web/dist && bun run build --force && bun run size`) at #282, against **223.44 kB** for `main` at `aad966e` the same hour: four numbers and eight array writes cost 0.06 kB, which is the floor this line can measure. 223.44 kB — measured the same way at #286, against **222.16 kB, 44.4%** for `main` at `8d69561` on a clean build the same hour. The 1.28 kB splits in two and the split is the thing to keep: **0.96 kB is Valve's six utility outlines** as polygon data, minus the hand-drawn set they replaced, measured at `d7284fe`; **0.32 kB is everything else**, because the weapon table has shipped since #260 for §5.3's row and the plate only became a reader of it. Drawing with art that is already in the bundle is what makes a resolution change cost nothing. The 222.16 kB figure and its history follow. 222.16 kB — measured the same way at #278, against **218.87 kB, 43.8%** for `main` at `bbb25f5` on a clean build the same hour. Of the 3.29 kB, 2.46 is `SlidingNumber` and the `react-use-measure` it depends on — both arrived with the registry in #281 and are spent here for the first time — and 0.82 kB is Valve's two armour outlines as polygon data, simplified by the same generator at the same tolerance as the weapons. **This line had read 195.08 kB since #134 and was a figure for no tree in between** — #281's token layer and #283's way in are the 23.79 kB it never recorded, and that is the defect #174 names rather than a lag. The 195.08 kB figure and its history follow. 195.08 kB — measured on a forced clean build (`rm -rf apps/web/dist && bun run build --force && bun run size`) at #134. The row had read **148.41 kB** since §6.1's plate states and was a figure for no tree in between: the 34 kB is every screen from #159's event feed to #262's weapon icons, and #174 is the issue that says a stale figure on this line is a defect rather than a lag. The largest single move inside it is measured — **170.85 kB → 182.53 kB** across #260, which is Valve's weapon outlines as 25 kB of polygon data, flattened at 0.3 of the icon's own 32-unit height; 0.6 would have been 16.7 kB of data and a 179.25 kB bundle, and one `TOLERANCE` in `tools/scripts/weapon-icons-generate.ts` is the whole difference. 182.61 kB at #261, 182.76 kB at #172, 182.75 kB at #264, 182.81 kB at #215, 182.91 kB at #268, 183.05 kB at #258 and 183.08 kB at #134 — so splitting a module costs about 0.1 kB when it adds functions (#172) and nothing at all when it only moves them (#264), a helper plus a per-glyph inline width costs the same 0.1 kB again, #258's 43-row bridge table costs 0.14 kB, and naming five transition properties where a class said `transition-all` costs 0.03 kB. Six consecutive changes inside 0.5 kB is the resolution this line has: quote it to two decimals, but do not read a story into the second one. **148.41 kB** since §6.1's plate states — 148.16 kB before them, and the 0.25 kB is what a blind disc, a death shrink, a selection ring, a stroked halo and the `LabelPass` seam they were split behind cost, which is what "the plate draws it, so nothing new is imported" looks like on this line. Measured on a forced clean build (`rm -rf apps/web/dist && bun run build --force`): #154 first recorded 148.31 kB here, which was the bundle one commit earlier — `bun run size` reports whatever `dist` currently holds, and nothing makes it notice that the tree moved underneath it (#155, #139). 148.16 kB since the §5 layout — 144.26 kB before it, and the 3.90 kB is `lucide-react`'s five corner icons plus the four cards, the glyph set and the ribbon. `lucide-react` is a runtime dependency approved under hard rule 10 on 12 August 2026; it is imported by name rather than through the per-icon deep paths the design system asks for, because the package ships ESM with `sideEffects: false` and the deep files carry no declarations — this number is what holds that claim to its word. **101.22 kB, 20.2% of budget** since #51 — the 3.56 kB is `@disa/demo-store`, which adds no runtime dependency: the container codec, the fingerprint and the two tiers are all platform APIs. 97.66 kB since #52 — 81.20 kB at the end of #22 plus the library screens. Of the 16.46 kB it added, 10 or so are `tailwind-merge` (105 kB raw), which arrives with the first `cn()` on screen and not again; the parse worker's own chunk, glue included, is 2.60 kB. Asserted by `bun run size`, which counts every non-locale chunk plus the heaviest locale chunk, gzip level 6, decimal kB — the unit Vite's build report uses. Since #93 it **refuses to report** when `apps/web/dist` holds two chunks of one family, because a Turborepo cache hit restores the directory without emptying it and never runs Vite, so a branch built there earlier leaves chunks that would be summed on top of the real ones — locally that read 206.31 kB against a true 108.87 kB. The fix it prints, `rm -rf apps/web/dist && bun run build`, is still a cache hit. |
| WASM binary | < 4 MB (CI fails above 24 MB) | tight gate as regression guard. Asserted by `wasm.yml` since #44. **`bun run size` weighs the binary in `dist` and refuses to weigh anything when that is not the one `crates/demo-parser-wasm/pkg/` holds** — the generated `pkg/` is not part of `@disa/web#build`'s cache key, so a rebuilt parser leaves the previous binary in `dist` under its old content hash and `rm -rf apps/web/dist && bun run build` restores it from the turbo cache without ever running Vite; measured on 12 August 2026, this row read 2.22 MB for a branch whose binary was 2.25 MB. The failure names both paths and both byte counts and asks for `bun run build --force`. `bun run size --wasm` reads `pkg/` alone by definition and does not compare. **2.66 MB, 66.5% of budget** since #66 built the shipped binary at `-O3`, up from 2.25 MB and 56.3% at `-Oz` — 0.41 MB of a cap that had 45% of itself unused, spent on the parse-time budget above, which had none. Measured after `bun run wasm:smoke` called into the binary: a size taken from an artifact that has never run is worthless (`docs/PARSER.md` §8). The figures below are all `-Oz` and are what each addition cost rather than what ships. 2.00 MB before #50; the 0.13 MB is the `js-sys` marshalling that hands the parsed demo over as JavaScript objects. 2.13 MB before #48; the 0.09 MB is the two container decoders (`docs/PARSER.md` §15). Phase 0 measured 2.18 MB at `-O`, which is between the two settings this repository has shipped and is why `-O3` was never the obvious loss of the cap it sounds like. |
...
3. **`backdrop-filter` exists on the two full-screen sheets and the dialog scrim, and nowhere
   ...
   **#315 is the first screen to spend it**, and only while the reader is zoomed in: the plate takes
   the stage's width and runs under the cards, which stay where they are with every reading on them.
   At 1× the rule still holds by construction — the plate is in a cell no card is in.
...
Go was never built. Query batching (§7.2) was settled at three passes, which upstream was read as
imposing as a floor; #355 found the flag that lets events ride with the tick columns, and it is two. Four consequences carry into Phase 2 — a pinned and patched upstream, output that differs
