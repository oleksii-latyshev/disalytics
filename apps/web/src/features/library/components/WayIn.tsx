 ...
 *
 * The ground under all of it is `WaveBackdrop` since #332 — the one place in the product where a
 * hue means nothing a demo said, and the only screen that may have one, because this is the screen
 * with no demo on it.
 *
...
export function WayIn({ state, onFile, onEnter, onSample, onClose }: Props) {
  ...
  return (
    <div className="app-shell relative grid grid-rows-[auto_minmax(0,1fr)] bg-surface-0 split:h-dvh split:grid-cols-[17.5rem_minmax(0,1fr)] split:grid-rows-1">
      <WaveBackdrop isLifted={isDraggedOver} />
