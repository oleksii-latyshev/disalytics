...
/**
 * The bottom of the screen. The round strip on top, then play/pause, the round timeline and the
 * speed control on one line — **92px in every state** since #279, where the layout it replaces spent
 * 205px on the same job and this one spent 108 whenever the survivor tracks were showing.
 *
 * That last figure is the one that mattered: the plate is sized from the row this block sits in, so
 * a block that could grow was a map that shrank under the reader. Nothing inside it changes its
 * height any more.
 *
 * **The strip is the top row.** It was flush to the card's bottom edge until #190, which put it as
 * far from the axis it seeks against as the card allowed; the reader's path is *pick a round, then
 * watch it*, and the two rows are adjacent for that reason. The pills carry their own gaps, so the
 * strip is padded like the row beneath it rather than running edge to edge.
 *
 ...
 * stage rather than being cut off by the card it sits in.
 *
 * **The scoreboard's brow is part of this block** and is in flow above it rather than over the
 * plate: the 32px are the block's own height, so the plate re-measures from 124px instead of 92px
 * and nothing is covered. Sending it over the plate instead is the reader's other position, and the
 * block goes back to 92px when they take it.
 *
 * **In fullscreen it leaves after three seconds of stillness** and the bottom 80px brings it back;
 * `useHotCorners` is what decides. Out of fullscreen it never hides.
 */
export function TimelineBlock({
  ...
  return (
    ...
    // control reachable by `Tab` — which is what brings it back. `prefers-reduced-motion` and the
    // settings sheet's motion row are the global reset's, which takes the duration to zero and
    // leaves the reveal instant rather than absent.
    <div
    ...
    >
      {hasScoreboard && (
        ...
        // it straight across a join that has to read as one shape. The strip's tooltip is `z-10`
        // and still hangs over this.
